...
const FindJobsPage = () => {
  ...
  // Get all unique locations from database for hero search matching
  const allLocationsFromDB = useMemo(() => {
  ...
  const handleHeroSearch = useCallback(() => {
    ...
    // Handle location
    if (locationTrimmed) {
      // Check if it matches any location from database (case-insensitive)
      const matchingLocation = allLocationsFromDB.find(
        (loc) => loc.toLowerCase() === locationTrimmed.toLowerCase()
      );
      ...
      if (matchingLocation) {
        ...
        // If not a matching location, include in search term (which searches location field)
  ...
  }, [heroSearchQuery, heroLocation, allLocationsFromDB]);

  // Helper functions to get unique values and counts dynamically from database
  const getUniqueLocations = useMemo(() => {
    // Extract unique locations directly from jobs data
  ...
  const getUniqueCategories = useMemo(() => {
    // Extract unique categories/teamNames directly from jobs data
    const categoryMap = new Map();
    
    jobs.forEach((job) => {
      // Check both teamName and category fields
      const category = job.teamName || job.category;
      if (category) {
  ...
  const getUniqueJobTypes = useMemo(() => {
    // Extract unique job types directly from jobs data
  ...
  const getUniqueExperienceLevels = useMemo(() => {
    // Extract unique experience levels directly from jobs data
    ...
    // Convert to array and sort by a logical order (if possible) or alphabetically
    return Array.from(experienceMap.entries())
      .map(([label, count]) => ({ label, count }))
      .sort((a, b) => {
        // Try to sort by experience level (years) if format is like "0-1 years", "2-4 years"
