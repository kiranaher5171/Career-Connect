...
const EditJobAlertPage = () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setFormData({
          ...
          emailNotifications: alert.emailNotifications !== undefined ? alert.emailNotifications : true,
          ...
          salaryRange: alert.salaryRange || { min: 0, max: 10000000 },
          skills: alert.skills || [],
        });
      }
    } catch (error) {
      console.error("Error fetching alert:", error);
      showSnackbar("Failed to load alert", "error");
    } finally {
      setLoading(false);
    }
  }, [alertId, router]);
  ...
  useEffect(() => {
    fetchAlert();
    fetchJobs();
  }, [fetchAlert, fetchJobs]);

  // Get unique values from jobs (same as create page)
  ...
  // Calculate matching jobs count (same logic as create page)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        showSnackbar("Job alert updated successfully!", "success");
        setTimeout(() => {
      ...
      showSnackbar(error.message || "Failed to update alert", "error");
    } finally {
      setSaving(false);
    }
  };
      ...
      <HeaderFooterLayout>
        <Container maxWidth="md">
          <Box className="page-content" mt={2}>
            <Box className="whitebx" sx={{ p: 3, textAlign: "center" }}>
              <CircularProgress />
        ...
        <Box className="page-content" mt={2}>
          <Box mb={3}>
            ...
            <Typography variant="body2" className="text-secondary">
              Update your job alert criteria
            </Typography>
          </Box>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <Grid container spacing={3}>
                ...
                <Grid size={{ xs: 12 }}>
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField {...params} label="Job Types" />
                    )}
                    renderTags={(value, getTagProps) =>
                      value.map((option, index) => (
                        <Chip label={option} size="small" {...getTagProps({ index })} />
                ...
                <Grid size={{ xs: 12 }}>
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField {...params} label="Experience Levels" />
                    )}
                    renderTags={(value, getTagProps) =>
                      value.map((option, index) => (
                        <Chip label={option} size="small" {...getTagProps({ index })} />
                ...
                <Grid size={{ xs: 12 }}>
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField {...params} label="Skills / Tags" />
                    )}
                    renderTags={(value, getTagProps) =>
                      value.map((option, index) => (
                        <Chip label={option} size="small" {...getTagProps({ index })} />
            ...
            <Box className="whitebx" sx={{ p: 3, mb: 3 }}>
              ...
              <Typography variant="body1">
                {matchingJobsCount} job{matchingJobsCount !== 1 ? "s" : ""} match
                your criteria
              </Typography>
            </Box>
...
export default EditJobAlertPage;
